...
from dynamic_Chaotic_Duffing import Dynamics
...
def run( seed = 123456, strategy = 'Random',  gpu=0, pool_num = None ):
    ...
    sampling_interval_min = [-2, -2.5]
    sampling_interval_max = [2, 2.5]
    ...
    test_labels = return_label_Chaotic_Duffing( test_points.to(device), dynamic=dynamic )
    ...
    first_labels = return_label_Chaotic_Duffing( select_points.to(device), dynamic=dynamic )
    print(first_labels)
    ...
    for index in range(al_epoches):
        ...
        sample_labels = return_label_Chaotic_Duffing( select_points.to(device), dynamic=dynamic )
...
if __name__ == '__main__':
    ...
    for s in ['Memory', 'Random']:  #
        ...
        for seed in range(1, 11):
            ACC.append(run(seed=seed, strategy=s, gpu=0, pool_num=1000))  # [al_epoches]
            print(ACC)
