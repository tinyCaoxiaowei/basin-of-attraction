...
from dynamic_Kuramoto import Dynamics
...
def run( seed = 123456, strategy = 'Random', pool_num = 1000 ):
    ...
    sampling_interval_min = [-10, -np.pi]
    sampling_interval_max = [7, np.pi]




    dynamic = Dynamics( steps=integrate_steps, step_size=integrate_step_size )
    dynamic_name = 'Kuramoto'
    ...
    test_labels = return_label_Kuramoto( test_points.to(device), dynamic=dynamic )
    ...
    first_labels = return_label_Kuramoto( select_points.to(device), dynamic=dynamic )
    print(first_labels)
    ...
    for index in range(al_epoches):
        ...
        sample_labels = return_label_Kuramoto( select_points.to(device), dynamic=dynamic )
...
if __name__ == '__main__':
    ...
    f1 = open(HOME + '/basin/fig/Kuramoto/result_mean_bl.txt', 'w')
    f1.write(str(MEAN))
    f1.close()
