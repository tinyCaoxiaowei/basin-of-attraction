...
from dynamic_Bistable import Dynamics
...
def run( seed = 123456, strategy = 'Random' ):
    ...
    sampling_interval_min = -50
    sampling_interval_max = 50
    ...
    # J = np.ones( [2,2] ) * 10
    J = np.array([[6.96469186, 2.86139335],
    ...
    x = np.arange( sampling_interval_min, sampling_interval_max + 0.1, 0.8 ).tolist()
    ...
    test_labels, label_value = return_label_Bistable( test_points.to(device), dynamic=dynamic, counts=True )
    ...
    first_labels = return_label_Bistable(select_points.to(device), dynamic=dynamic, label_value=label_value )
    print(first_labels)
    ...
    for index in range(al_epoches):
        ...
        pool_x = np.random.uniform(sampling_interval_min, sampling_interval_max, pool_num)
        pool_y = np.random.uniform(sampling_interval_min, sampling_interval_max, pool_num)
        ...
        sample_labels = return_label_Bistable(select_points.to(device), dynamic=dynamic, label_value=label_value )
