...
from dynamic_Duffing import Dynamics
...
def run( seed = 123456, strategy = 'Random', pool_num = 1000 ):
    ...
    sampling_interval_max = 2





    dynamic = Dynamics( step_size = integrate_step_size, steps = integrate_steps, K=0.5 )
    dynamic_name = 'Duffing'
    ...
    x = np.linspace( sampling_interval_min, sampling_interval_max, 150 ).tolist()
    ...
    test_labels = return_label_Duffing( test_points.to(device), dynamic=dynamic )
    ...
    # select_points_y = np.array( [ (sampling_interval_min + sampling_interval_max) / 2 ] * select_points_x.shape[0] )
    ...
    first_labels = return_label_Duffing( select_points.to(device), dynamic=dynamic )
    print(first_labels)
    ...
    for index in range(al_epoches):
        ...
        if strategy != 'BADGE':
            ...
            if strategy == 'Coreset':
                ...
                # kmeans = KMeans(n_clusters=al_sample_nums, init='k-means++', random_state=0, n_init="auto").fit( logits.cpu().numpy() )
                # centers = kmeans.cluster_centers_.tolist() #[al_sample_nums, class_num ]
                # indices = [ j in centers for j in pool.tolist() ]
            elif strategy == 'Conf':
            ...
            # param = torch.cat( [ model.classifier.state_dict()['weight'].reshape(-1),
            #                      model.classifier.state_dict()['bias'].reshape(-1)] )
            grad = []
            for l in loss:
            ...
            # kmeans = KMeans(n_clusters=al_sample_nums, init='k-means++', random_state=0, n_init="auto").fit(grad.cpu().numpy())
            # centers = kmeans.cluster_centers_.tolist()  # [al_sample_nums, class_num ]
            # indices = [j in centers for j in pool.tolist()]
        ...
        sample_labels = return_label_Duffing( select_points.to(device), dynamic=dynamic )
...
if __name__ == '__main__':
    ...
    f1 = open(HOME + '/basin/fig/Duffing/result_mean_bl.txt', 'w')
    f1.write(str(MEAN))
    f1.close()
    #
