...
class Dynamics():
    ...
    def integrate( self, input_ ): #[bs,nodes*2]
        ...
        x, y, z = input_[:, 0:1 ], input_[:, 1:2 ], input_[:, 2:3 ] #[bs,1], [bs,1]

        d_x = self.c1 * ( z - x -
                          ( self.m1 * x + (self.m0 - self.m1) * (torch.abs(x+1)-torch.abs(x-1)) /2 ) #g(x)
                          ) #[bs,1]

        d_y = self.c2 * ( x -y + z ) #[bs,1]
        d_z = - self.c3 * y

        return torch.cat( [d_x, d_y, d_z], dim=-1 ) #[[bs,1],[bs,1]] -- [bs,3]
...
if __name__ == "__main__":
    ...
    x = np.linspace(-2., -0.5, 150).tolist()
    y = np.linspace(-2.5, 2, 150).tolist()
    ...
    plt.scatter( x, y, c=label.numpy(), cmap='Pastel1', s=5, vmin=0, vmax=1 )
